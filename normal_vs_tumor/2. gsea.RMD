...
ggplot(GSEA_p, aes(pval, reorder(pathway, padj)))+
  geom_point()
...
x = which(GSEA_p$pathway == "METABOLISM_OF_XENOBIOTICS_BY_CYTOCHROME_P450")

for(i in 1:z){
  # select genes from one pwathway
x =  which(GSEA_p$pathway == "METABOLISM_OF_XENOBIOTICS_BY_CYTOCHROME_P450")# choose pathway by position
...
plot.v <- ggplot(luad.info, aes(y = abs(spval), diff.mean, colour = expr, label = name))+
  ...
  geom_point(data = selection,
             aes(y = abs(spval), diff.mean, color = pathway), size = 4)+
  geom_text_repel(data = selection,
                  aes(diff.mean, abs(spval), label = rownames(selection)),
                  size = 5, hjust = -.01, col = "black")
...
grid.arrange(grobs = plots$pw.v.plots[pathways[6:10]])
```
