...
Values = ssorted_p_log10$spval
names(Values) = rownames(ssorted_p_log10)
...
p2
```
```{r select only most down- and up-regulated pw's}
most_up_down = as.data.frame(rbind(head(reg2, 5), tail(reg2, 5)))
View(most_up_down)

pathways = rownames(most_up_down)
plots$pw.v.plots[pathways[1:3]]

grid.arrange(grobs = plots$pw.v.plots[pathways[1:5]])
```
