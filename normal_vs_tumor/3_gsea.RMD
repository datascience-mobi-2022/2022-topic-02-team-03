...
pw = names(genesets$genesets[x])
pw

pw.genes = unlist(genesets$genesets[x], use.names = F)#genes in the pathway that are at the leading edge
...
plot.v <- ggplot(luad.info, aes(y = abs(spval), diff.mean, colour = expr, label = name))+
  ...
  geom_text_repel(data = selection,
                  aes(fc, -log10(p.wilc), label = rownames(selection)), size = 5, hjust = -.01, col = "black")
