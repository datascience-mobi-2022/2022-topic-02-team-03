...
for(i in 1:46){
  ifelse(1 > reg$up_div_down[i] & reg$up_div_down[i] > 0,
         reg$singed_ratio[i] <- reg$up_div_down[i]*(-10),
...
s = ggplot(reg, aes(fill= regulation, x= singed_ratio,
                              y=rownames(reg)))+ 
  scale_fill_manual(breaks = c("nur.über.expr", "über.expr", "nur.unter.expr", "unter.expr"),
                      values = c("violetred4", "thistle1", "purple4", "lightblue"))+
  geom_bar(position="dodge", stat="identity")+
