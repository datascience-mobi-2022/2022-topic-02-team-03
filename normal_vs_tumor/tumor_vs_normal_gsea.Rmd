...
#normaler v.plot
plot <- ggplot(luad.info, aes(y = abs(spval), diff.mean, colour = expr, label = name))+
  geom_point(size = .2)+
...
plots[[i]] <- plot #save the plot in the list element
message("plot saved")
...
names(plots) <- paste0(name, 1:length(plots))
pw.v.plots = plots
...
rm(plots, z, x, pw, pw.genes, name, genes, positions, selection, col, i)
```

```{r print plot}
grid.arrange(grobs = (pw.v.plots[1:15]))
grid.arrange(grobs = (pw.v.plots[16:30]))
grid.arrange(grobs = (pw.v.plots[31:46]))
```
