...
reg$regulation[which(reg$singed_ratio > 0)] <- "über.expr"
reg$regulation[which(reg$singed_ratio < 0)] <- "unter.expr"
reg$regulation[which(reg$singed_ratio == Inf)] <- "nur.über.expr"
reg$regulation[which(reg$singed_ratio == -Inf)] <- "nur.unter.expr"
reg$regulation[which(reg$singed_ratio == 1)] <- "gleich"
View(reg)
s = ggplot(reg, aes(fill= regulation, x= singed_ratio,
                              y=rownames(reg)))+ 
  scale_fill_manual(breaks = c("nur.über.expr", "über.expr", "nur.unter.expr", "unter.expr", "gleich"),
                      values = c("royalblue4", "orangered", "royalblue4", "royalblue1", "green"))+
  geom_bar(position="dodge", stat="identity")+
