...
x = ggplot(reg2, aes(x = mean, y = order, size = number_of_genes, col = regulation))+
  geom_point(stat = "identity")+
  scale_color_manual(breaks = r,
                     values = col)+
  labs(title = paste("dotplot"), x = "mean expression rate (fold changes) of each pw", y = "pathways")+
  theme(axis.text = element_text(size = 7))+
  scale_y_discrete(limits = rownames(reg2))# to get the pws in the correct order somehow...

y = ggplot(reg2, aes(fill= regulation, x = mean,
                              y=order))+ 
  scale_fill_manual(breaks = r, # regulation information
                      values = col)+
  geom_bar(position="dodge", stat="identity")+
  labs(title = paste("more up or more down reg"))+
  theme(axis.text = element_text(size = 5))+
  labs(title = paste("barplot"), x = "mean expression rate (fold changes) of each pw", y = "pathways")+
...
p <- ggplot(boxplot_df, aes(x=gene_expr, y=pathway)) + 
  geom_boxplot(fill = "lightskyblue", col="midnightblue",
               outlier.colour="red4", outlier.shape=20,
               outlier.size=2)+
  labs(title = paste("boxplot"), subtitle = "some genes are highlighted", x = "mean expression rate (fold changes) of each pw", y = "pathways")+
