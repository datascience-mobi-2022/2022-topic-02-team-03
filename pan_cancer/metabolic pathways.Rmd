...
## Creating a list of our chosen metabolism gene sets
...
Calculating the Jaccard-Coefficient for our gene sets:
...
### inoffiziell: Überprüfung mit Wangs Metabolischen Pathways

```{r}
Jaccard_coefficients_W = matrix(ncol = length(genesets[[1]]), nrow = length(metabolic_pathways_Wangjun), byrow = TRUE)
for (i in (1:length(metabolic_pathways_Wangjun))){
  for (j in (1:length(genesets[[1]]))){
       j_value_W = jaccard(metabolic_pathways_Wangjun[[i]], genesets[[1]][[j]])
    Jaccard_coefficients_W[i, j] = j_value_W 
  }}

colnames(Jaccard_coefficients_W) = names(genesets[[1]])
rownames(Jaccard_coefficients_W) = names(metabolic_pathways_Wangjun)

Jaccard_coefficients_W
...
pheatmap(Jaccard_coefficients_W, cluster_rows = FALSE, cluster_cols = FALSE, fontsize = 5, color = colorRampPalette(c("lightskyblue1", "firebrick"))(100))
setHook("grid.newpage", NULL, "replace")
grid.text("hallmarks_genesets", y=1, x=0.3, gp = gpar(fontsize=8))
grid.text("metabolic_pathways_Wangjun", x=0, y=0.6, rot=90, gp = gpar(fontsize=8))
```
