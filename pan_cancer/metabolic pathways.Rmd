...
```{r}
Jaccard_coefficients = matrix(ncol = length(genesets[[1]]), nrow = length(metabolism_list_gs), byrow = TRUE) #creating an empty matrix 
for (i in (1:length(metabolism_list_gs))){
  ...
  }} #storing the jaccard coefficients in the matrix
...
```{r}
pheatmap(Jaccard_coefficients, cluster_rows = FALSE, cluster_cols = FALSE, fontsize = 4, color = colorRampPalette(brewer.pal(n = 7, name ="BuPu"))(100), main = "Jaccard Coefficients of hallmark genesets and metabolism genesets")
```
...
Visualizing the similarity using a heatmap:
```{r}
pheatmap(Jaccard_coefficients_hallmark, cluster_rows = FALSE, cluster_cols = FALSE, fontsize = 4, color = colorRampPalette(brewer.pal(n = 7, name ="BuPu"))(100), main = "Jaccard Coefficients of given and hallmark genesets")
```
...
```{r}
genesets_id = list() #creating an empty list
...
for (i in (1:length(metabolism_list_id))){
  for (j in (1:length(genesets_id))){
       j_value = jaccardSets(metabolism_list_id[[i]], genesets_id[[j]])
...
```{r}
pheatmap(Jaccard_coefficients_id, cluster_rows = FALSE, cluster_cols = FALSE, fontsize = 4, color = colorRampPalette(brewer.pal(n = 7, name ="BuPu"))(100), main = "Jaccard Coefficients of hallmark genesets and metabolism genesets with ensemble id")
```
