...
A first rough model was trained using all 20 of the chosen genes and the glm function [@R-base] and specifying the model to use a binomial error distribution and a logit link. The blorr package was used to dtermine the best composition of the given 20 genes [@blorr]. With the best configuration the final model was trained on the training dataset.

### Performance measurments

To evaluate the model the first step was to predict whether the patients of the testing dataset were LUAD patients. For this the native predict function [@R-base] was used. The resulting probabilities were transformed into predictions for 1 or 0 using a cutoff value of 50%. Next a confusion table was used to estimate the false-positive and false-negative rates using the known tumor type of each patient and comparing that to the predicition of the model. As a final evaluation step the package ROCR [@rocr] was used to create a ROC curve clearly showing the performance of the model.
...
In order to illustrate which pathways of the hallmark and metabolism genesets are most up or down regulated, a barplot was created (\textbf{Fig. \ref{fig:GSEA barplot}}). The genes crucial to the mean expression of a pathway are the ones that are in the leading edge of the GSEA result.
...
### Visualisation of TCGA patients of different tumor types
...
The logistic model that was trained on the TCGA expression dataset. The model's goal was to predict whether a cancer patient suffers from LUAD or not. In order to be used reliably, the model has to be precise enough. Testing of our model revealed the following characteristics: The model predicts 136 LUAD patients correctly, as well as 2752 non-LUAD cases. Also shown in the confusion table (\textbf{Fig. \ref{fig:confusionmatrix}}) are the 27 false-negative occurrences and 7 false-positive occurrences. This results in an accuracy of 0.986 %.
