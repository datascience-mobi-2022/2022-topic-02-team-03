...
To this day, lung cancer is the leading cause of cancer death worldwide [@zhang2020]. Lung adenocarcinomam (LUAD) is a form of non-small cell lung cancer which accounts for approximately 40% of lung cancer cases [@wang2020]. This cancer is characterised by a particularly high mortality rate and a remarkably low 5-year overall survival rate of merely 18% [@li2022]. In theory, every cell is capable of developing into a cancer cell through acquisition of so-called hallmark capabilities that essentially cause metabolic reprogramming, immune evasion and uncontrolled proliferation through numerous genetic mutations [@hanahan2011\]\[@peng2018]. In order to gain an insight into which mutations drive cancer development and how to best treat different cancer types, one must start by deciphering the intricate workings of gene expression regulation within a tumor cell.
...
To assess how the pathway expression differs between tumor and normal tissue, a GSEA was performed using the fgsea package [@fgsea]. As input for the genes the signed ranked list and for the pathways the hallmark and the metabolism genesets were used (87 genesets). An enrichment score and a leading edge containing the genes that contribute most to the enrichment score were calculated for each pathway.
...
In the following, a GSVA was performed using the gsva package [@gsva] to illustrate the pathway expression of each patient. For this the gene ontology genesets (C5 BP) with about 3000 pathways were used.

For each pathway of the resulting pathway enrichment matrix the standard deviation was calculated. Those with the greatest standard deviation were retained.

The selected pathways and their expression rate for each patient were then shown in a heatmap using the package ComplexHeatmap [@complexheatmap].

After the shortened matrix was divided into tumor and normal tissue, the foldchange of each pathway was calculated. This was calculated taking the difference between the mean expression of each pathway in tumor and normal tissue. In addition, the Wilcoxon tests for dependent samples was applied to each pathway. This was illustrated in a volcano plot using the package ggplot [@gplots] in which the -log10(p-values) were plotted against the foldchanges.

Finally, volcano plots were created for selected pathways to see which genes were over expressed and under expressed in the individual pathways, for literature comparisons.
...
```{r Jaccard, echo = FALSE, message=FALSE, fig.align='center', fig.cap='\\textbf{Jaccard Coefficients of hallmark gene sets and metabolism gene sets.} The x-axis is defined by the given hallmark gene sets, whereas the y-axis is assigned to the selected metabolism geneset.', out.width='0.75\\linewidth'}
knitr::include_graphics("C:\\Users\\Marie Kleinert\\Documents\\GitHub\\GitHub-Ordner DataAnalysis\\output\\2_geneset_heatmap_metabolism_vs_hallmarks.pdf")
```
...
Dimension reduction of the cleaned data conducted by performing PCA and UMAP results in the plot shown in **Fig. XXX**. To identify clustering of different cancer types, the data points of each patient was colored accordingly. Based on the 33 different types occurring in the dataset, the reduced data results in approximately 16 clusters. Patients suffering from LUAD show a similar gene expression indicated by the isolated, turquoise cluster in the right, bottom corner.
...
knitr::include_graphics("C:\\Users\\Marie Kleinert\\Documents\\GitHub\\GitHub-Ordner DataAnalysis\\output\\3_gsva_heatmap_C5.pdf")
```
