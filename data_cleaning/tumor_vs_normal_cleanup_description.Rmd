...
rm(mmv_df, luad.normal, luad.tumor, genenames, p, x, zero.varr, rows, luad.normal.new, luad.tumor.new, mmv_df_new)
```
```{r remove biotypes}
#---------------------------------------------#
...
for(i in 1: ncol(luad.normal.clean)){
  p.wilc[i] = c(wilcox.test(luad.tumor.clean[,i],luad.normal.clean[,i], paired = TRUE, alternative = "two.sided", exact = FALSE )$p.value)
}
...
for (i in 1:nrow(luad.info)){ #setting up expression
  ifelse((   (luad.info$diff.mean[i] > 0)   ),
         luad.info$spval[i] <- -log10(luad.info$p.wilc[i]),
         luad.info$spval[i] <- log10(luad.info$p.wilc[i]))
}
View(luad.info)
...
```{r pathways im volcano plot anfärben}
genesets <- readRDS("./data/hallmarks_genesets.rds")
...
names(genesets$genesets[x])
# select genes from one pwathway
x = which((GSEA_p$pathway) == "Meta_AACR")
pathway = unlist(GSEA_p$leadingEdge[x], use.names = F)
genes = luad.info$name
pw.genes = sort(match(pathway, genes))
pw.genes
selection = luad.info[pw.genes,]
selection$pathway = "Meta_AACR"


sp5 = ggplot(luad.info, aes(y = abs(spval), diff.mean, colour = expr, label = name))+
  geom_point(alpha = .5)+
  scale_color_manual(values=c("lightblue", "lightgray", "lightblue") )+
  labs(title = paste("normal volcano plot pw: ", "Meta_AACR"), y = "fc", x = "pwilc")+ 
  geom_point(data = selection,
             aes(y = abs(spval), diff.mean), color = "red", size = 3)
sp5 



luad.info$number = c(1:dim(luad.info)[1])


View(luad.info[is.element(luad.info$name, GSEA_p$leadingEdge[1]),])
View(luad.info[rownames(luad.info) %in% GSEA_p$leadingEdge[1], ])
filter(luad.info, name %in% GSEA_p$leadingEdge[1])
```
